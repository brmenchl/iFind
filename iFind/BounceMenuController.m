...
@property (nonatomic, retain) UIImageView *vertBar;
...
- (id)init
{
    ...
    if (self) {
        ...
        [self.view addSubview:self.vertBar];
...
- (void)viewWillLayoutSubviews {
    ...
    CGFloat buttonStartX = 80;
    for (int i = 0; i < self.buttons.count; i++) {
        UIButton *button = [self.buttons objectAtIndex:i];
        CGFloat buttonX = buttonStartX + (i * 50);
        button.frame = CGRectMake(buttonX, self.view.bounds.size.height - 45, 30, 30);
    }
}
...
- (void)openMenu {
    ...
    for (int i = 0; i < self.viewControllers.count; i++) {
        ...
        CGFloat buttonX = buttonStartX + (i * 50);
        controllerButton.frame = CGRectMake(buttonX, self.view.bounds.size.height + 40, 30, 30);
        ...
        finalFrame.origin = CGPointMake(buttonX, 568);
        [UIView animateWithDuration:0.3 delay:(0.0+(i*0.07)) options:UIViewAnimationOptionCurveEaseOut animations:^{
            controllerButton.frame = finalFrame;
        }
        completion:nil];
        
        [UIView animateWithDuration:0.4 delay:(0.15+(i*0.07)) options:kNilOptions animations:^{
    ...
    openedPosition.origin = CGPointMake(0, -70);
    ...
    completion:^(BOOL finished) {
        [UIView animateWithDuration:0.2 delay:0.0 options:UIViewAnimationOptionCurveEaseIn animations:^{
            self.contentView.frame = CGRectOffset(openedPosition, 0, 10);
        }
        completion:^(BOOL finished) {
            self.isAnimating = NO;
        }];
